...
class BouabidVu2026BehaviorInterface(BaseTemporalAlignmentInterface):
    """
    Interface for reading the processed behavior data from .mat files from the Howe Lab.
    """

    display_name = "BouabidVu2026Behavior"
    ...
    def __init__(
        ...
        data_field : str 
            Field within the processed data .mat containing the behavior data.
        verbose : bool, default: True
            controls verbosity.
        """

        super().__init__(file_path=file_path, data_field=data_field, verbose=verbose)
        self._timestamps = None

    def get_original_timestamps(self) -> np.ndarray:
        behavior_data = read_mat(filename=self.source_data["file_path"],variable_names=self.source_data["data_field"])
        behavior_data = behavior_data[self.source_data["data_field"]]
    ...
    def get_metadata(self):
        ...
        metadata["Behavior"] = dict(
            TimeSeries=[
                ...
                dict(
                    name="WheelVelocity",
                    description="Velocity on the linear treadmill wheel, measured in m/s.",
                    unit="m/s",
                ),
            ],
            TimeIntervals=dict(
                name="TimeIntervals",
                description="Some sessions may include water reward delivery, where a water spout mounted on a post delivered water rewards (9 μL) at random time intervals (randomly drawn from a 5-30s uniform distribution) through a water spout and solenoid valve gated electronically. Some session include optogenetic midbrain stimulation during unpredicted reward recordings. Some sessions include a Pavlovian task with 2 cues: visual (blue LED) or auditory (12 kHz tone) stimuli at random intervals (4–40 s) and rewarded at 100% during the learning phase, or 80/20% during the extinction phase (rewarded and extinguished cue, respectively). Licking was monitored by a capacitive touch circuit connected to the spout."
            ),
        )
    ...
    def add_velocity_signals(
        ...
        nwbfile: NWBFile,
        metadata: dict,
        treadmill_diameter_in_meters: float = None,
        stub_test: Optional[bool] = False,
    ) -> None:
        """
        In the case of the ball treadmill, add the velocity signals (yaw (z) velocity and the roll 
        and pitch (x, y)) to the NWBFile. The yaw velocity is converted to radians/s using the 
        calculation provided from the Howe Lab. The roll and pitch velocities are in m/s.
        In the case of the linear wheel treadmill, add the velocity (m/s).
        ...
        treadmill_diameter_in_meters : float, optional
            The diameter of the treadmill ball or wheel in meters, optional.
            Used to convert the yaw velocity from m/s to radians/s, when not specified default value is 0.2032.
            On the wheel, the default value is 0.1905.
        ...
        behavior = get_module(
            nwbfile,
            name="behavior",
            description="In the case of the ball treadmill, contains the velocity signals from two optical mouse sensors (Logitech G203 mice with hard plastic shells removed). In the case of the wheel treadmill, contains the velocity signals from the rotary encoder.",
        )

         
        if "ballYaw" in behavior_data: # if yaw is there, roll and pitch are too
            # ballYaw is in m/s convert to radians/s using their code
            # https://zenodo.org/records/10272874 ball2xy.m
            treadmill_diameter_in_meters = treadmill_diameter_in_meters or 0.2032
            yaw_in_meters = behavior_data["ballYaw"][:end_frame]
            yaw_in_radians = yaw_in_meters / (np.pi * treadmill_diameter_in_meters) * 2 * np.pi
        else:
            yaw_in_radians = behavior_data["timestamp"]*0 # zeros
        ...
        if "ballRoll" in behavior_data and "ballPitch" in behavior_data:        
            ...
            velocity_in_meters = behavior_data["timestamp"]*0 # zeros
        ...
        if "rotaryEncoderVelocity" in behavior_data:
            treadmill_diameter_in_meters = treadmill_diameter_in_meters or 0.1905
            wheel_vel = behavior_data["rotaryEncoderVelocity"][:end_frame]
        else:
            wheel_vel = behavior_data["timestamp"]*0 # zeros
        wheel_velocity_metadata = time_series_metadata[2]
        wheel_velocity = TimeSeries(
            **wheel_velocity_metadata,
            data=wheel_vel,
            timestamps = timestamps,
        )
        behavior.add(wheel_velocity)
    ...
    def add_binary_signals(self, nwbfile: NWBFile, metadata: dict):
        ...
        event_name_mapping = dict(
            ...
            lick="Lick",
            reward="Reward",
            stimDriver="Optogenetic Stimulation"
        )
        ...
        for event_name in event_name_mapping.keys():
            ...
            # for stimDriver, change to binary
            if event_name=="stimDriver":
                event_data = (behavior_data["stimDriver"]>0.25).astype(int)
            else:
        ...
        events.add_column(
            name="event_type",
            description="The type of event (licking, light delivery, tone delivery, reward delivery, optogenetic stimulation delivery).",
        )
    ...
    def add_to_nwbfile(
        ...
        metadata: dict,
        stub_test: Optional[bool] = False,
        treadmill_diameter_in_meters: Optional[float] = None,
    ) -> None:
        ...
        self.add_velocity_signals(
            ...
            metadata=metadata,
            stub_test=stub_test,
            treadmill_diameter_in_meters=treadmill_diameter_in_meters,
        )
