...
from howe_lab_to_nwb.bouabid_vu_2026.utils import add_fiber_photometry_series
from howe_lab_to_nwb.bouabid_vu_2026.utils.add_fiber_photometry import add_fiber_photometry_table


class BouabidVu2026FiberPhotometryInterface(BaseTemporalAlignmentInterface):
    """
    Interface for reading fiber photometry data from .mat files from the Howe Lab.
    """

    display_name = "BouabidVu2026FiberPhotometry"
    ...
    def __init__(
        ...
        super().__init__(
            file_path=file_path, data_field=data_field, ttl_file_path=ttl_file_path, ttl_stream_name=ttl_stream_name, verbose=verbose
        )
    ...
    def add_to_nwbfile(
        ...
        fiber_photometry_data = read_mat(filename=self.source_data["file_path"],variable_names=self.source_data["data_field"])
        fiber_photometry_data = fiber_photometry_data[self.source_data["data_field"]]
        ...
        if "F" not in fiber_photometry_data:
            raise ValueError(f"Expected raw fluorescence 'F' is not in '{self.source_data['file_path']}': '{self.source_data['data_field']}'.")
        ...
        # Add baseline fluorescence data to NWBFile
        if "F_baseline_exp" in fiber_photometry_data:
            baseline_fluorescence = fiber_photometry_data["F_baseline_exp"]
            baseline_data_to_add = baseline_fluorescence if not stub_test else baseline_fluorescence[:100]
            fluorescence_metadata = deepcopy(metadata["Ophys"]["FiberPhotometry"]["FiberPhotometryResponseSeries"][0])
            fiber_photometry_series_name = f"Baseline{fluorescence_metadata['name']}"
            description = fluorescence_metadata["description"]
        ...
        # Add baseline corrected fluorescence data to NWBFile
        if "Fc_exp" in fiber_photometry_data:
            corrected_fluorescence = fiber_photometry_data["Fc_exp"]
            corrected_data_to_add = corrected_fluorescence if not stub_test else corrected_fluorescence[:100]
            fluorescence_metadata = deepcopy(metadata["Ophys"]["FiberPhotometry"]["FiberPhotometryResponseSeries"][0])
            fiber_photometry_series_name = f"DfOverFexp{fluorescence_metadata['name']}"
            description = fluorescence_metadata["description"]
            description = description.replace("Raw", "Baseline corrected (DF/F)")
            fluorescence_metadata.update(
                name=fiber_photometry_series_name,
