...
def convert_single_dual_wavelength_session_from_table(
    ...
    filtered_data_table = data_table[(data_table["Mouse"].astype(str)==subject_id)]
    filtered_data_table = filtered_data_table[filtered_data_table["Experiment Directory"].astype(str)==exp_dir]
    ...
    raw_imaging_file_paths=[session_folder_path /  "raw" / filtered_data_table["Raw Imaging File: green"].values[0], session_folder_path /  "raw" / filtered_data_table["Raw Imaging File: red"].values[0]]
    processed_data_file_path = session_folder_path / filtered_data_table["Processed Data File"].values[0]
    fiber_photometry_fields = [filtered_data_table["Processed Photometry Field: green"].values[0],filtered_data_table["Processed Photometry Field: red"].values[0]]
    behavior_fields = [filtered_data_table["Processed Behavior Field: green"].values[0],filtered_data_table["Processed Behavior Field: red"].values[0]]
    index_fields = [filtered_data_table["Processed Index Field: green"].values[0],filtered_data_table["Processed Index Field: red"].values[0]]
    fiber_locations_file_path = subject_folder_path / "fiber_table.xlsx"
    excitation_wavelengths_in_nm = [filtered_data_table["LED Excitation Wavelength (nm): green"].values[0],filtered_data_table["LED Excitation Wavelength (nm): red"].values[0]]
    indicators = [_get_indicator_from_aav_string(filtered_data_table["Relevant Injected Sensor: green"].values[0]),_get_indicator_from_aav_string(filtered_data_table["Relevant Injected Sensor: red"].values[0])]
    ttl_file_path = session_folder_path / "raw" / filtered_data_table["Raw Behavior File"].values[0]
    ...
    if not (nwbfile_path.exists() and not overwrite):
...
if __name__ == "__main__":
    ...
    convert_single_dual_wavelength_session_from_table(
