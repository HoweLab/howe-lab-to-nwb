...
class BouabidVu2026SegmentationExtractor(SegmentationExtractor):
    extractor_name = "BouabidVu2026Segmentation"

    def __init__(self, file_path: str, data_field: str, sampling_frequency: float, accepted_list: list[int] = None):
        """
        Create a BouabidVu2026SegmentationExtractor instance from a .mat file that contains the ROI locations and image masks.
        """
        super().__init__()
        self.file_path = file_path
        self.data_field = data_field

        data = read_mat(file_path,variable_names=data_field)
